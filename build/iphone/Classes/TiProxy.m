...
//Rare exceptions to indicate a bug in the _sissionspaint code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_sissionspaint = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_sissionspaint]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.sissionspaint.module"
