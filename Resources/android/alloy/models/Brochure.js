	...
	extendCollection: function(Collection) {
		_.extend(Collection.prototype, {
			...
			addBrochure : function(b_id,b_title,cover,attachment, b_url,b_status,b_format) {
                ...
                if (res.isValidRow()){
             		sql_query = "UPDATE " + collection.config.adapter.collection_name + " SET title='"+b_title+"', cover='"+cover+"', content='"+attachment+"', status='"+b_status+"' WHERE id='" +b_id+"'";
                }else{
                	sql_query = "INSERT INTO " + collection.config.adapter.collection_name + " (id, title, cover,content, url,status, isDownloaded,format ) VALUES ('"+b_id+"','"+b_title+"','"+cover+"','"+attachment+"','"+b_url+"','"+b_status+"', 0, '"+ b_format+"')" ;
				}
